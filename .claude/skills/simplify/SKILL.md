---
name: simplify
description: Review the changed code for reuse, simplification, efficiency, altitude, and comment-hygiene cleanups, then apply the fixes. Quality only — it does not hunt for bugs; use /code-review for that. Does NOT commit/push — to ship, use /ship (which runs this first).
---

`/simplify → 5 cleanup agents in parallel → apply the fixes`

You are improving the quality of the changed code, not hunting for bugs. Review
it for reuse, simplification, efficiency, altitude, and comment hygiene, then fix
what you find. Do not look for correctness bugs — that is what `/code-review` is
for.

> Skill này CHỈ review + sửa, KHÔNG tự commit/push. Muốn đưa lên repo thì dùng
> `/ship` (nó tự chạy `/simplify` trước rồi mới commit + push).
...
Launch **5 independent review agents** via the Agent tool, all in a
single message so they run concurrently. Pass each agent the diff and one of
the five angles below. Each returns its findings with `file`, `line`, a
...
Enforce the project convention: **comments are for WHY, not WHAT.** Names of
functions/variables carry WHAT; the code itself shows the mechanism. Flag and
remove/rewrite:

- Comments that narrate what the next line(s) do or restate the code in prose
  ("Detect the pairs", "Loop over rows", "Set the flag", "Transfer legs first").
- Section-label comments whose block is already obvious from the code.
...
Skill kết thúc ở đây. Việc commit/push (nếu có) do người dùng yêu cầu và thực
hiện ở bước riêng — skill này không tự làm.
